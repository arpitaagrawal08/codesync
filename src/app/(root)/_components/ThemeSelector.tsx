...
const THEME_ICONS: Record<string, React.ReactNode> = {
  "vs-dark": <MoonIcon className="size-4" />,
  "vs-light": <SunIcon className="size-4" />,
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="w-40 group relative flex items-center gap-2 px-3 py-1.5 bg-[#1e1e2e]/80 hover:bg-[#262637] 
        rounded-md transition-all duration-200 border border-gray-800/50 hover:border-gray-700"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity" />

        <Palette className="w-4 h-4 text-gray-400 group-hover:text-gray-300 transition-colors" />

        <span className="text-gray-300 min-w-[60px] text-left group-hover:text-white text-sm transition-colors">
          {currentTheme?.label}
        </span>

        <div
          className="relative w-3.5 h-3.5 rounded-full border border-gray-600 group-hover:border-gray-500 transition-colors"
          ...
          >
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                className={`relative group w-full flex items-center gap-3 px-3 py-2 hover:bg-[#262637] transition-all duration-200
              ...
              >
                ...
                <div
                  className="relative size-3.5 rounded-full border border-gray-600 
